...
export function addLocalMessage(obj) {
  ...
  if (!createGroup) {
    !found && store.dispatch(SessionActions._pushSession({
  ...
  if (firstCrate && !found) {
    if (addType === 'contactAdded') {
      ...
      var formatMsg = formatLocalMessage(who, chatType, _message, msgType || 'txt', false);
...
export default function createlistener(props) {
  WebIM.conn.addEventHandler("EaseChat", {
    onConnected: function onConnected(msg) {
      // init DB
      AppDB.init(WebIM.conn.context.userId); // get session list
    ...
    onTextMessage: function onTextMessage(message) {
      ...
      if (ext.action === 'invite') {
        ...
        store.dispatch(MessageActions.addMessage(_message2));
      } else {
    ...
    onDeliveredMessage: function onDeliveredMessage(message) {
      ...
      var mid = message.mid,
          id = message.id;
      store.dispatch(MessageActions.updateMessageStatus(message, "received", id, mid));
    },
    ...
    onGroupChange: function onGroupChange(event) {
      ...
      if (event.type === 'direct_joined') {
        ...
        var joinedGroup = store.getState().session.joinedGroups;
        ...
        var _storeSessionList = store.getState().session;
      ...
      if (event.type === 'addAdmin' || event.type === 'removeAdmin' || event.type === 'changeOwner') {
        var _uikit_store$getState = uikit_store.getState().global.globalProps,
            chatType = _uikit_store$getState.chatType,
            _to = _uikit_store$getState.to;

        if (chatType === 'groupChat' && _to === event.gid) {
          store.dispatch(ThreadActions.getCurrentGroupRole({
            chatType: chatType,
            to: _to
          }));
        }
    ...
    onContactDeleted: function onContactDeleted(msg) {
      ...
      store.dispatch(GlobalPropsActions.setGlobalProps({
        to: null
      }));
    },
    ...
    onCmdMessage: function onCmdMessage(msg) {
      ...
      switch (action) {
        case 'TypingBegin':
          store.dispatch(GlobalPropsActions.setShowTyping({
            showTyping: true
          }));
          break;
