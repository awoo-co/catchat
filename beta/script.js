  ...
  try {
    ...
    request.onsuccess = async () => {
      ...
      if (usePuter) {
        ...
        localStorage.setItem('catchat-beta-last-backup-path', savedPath);
        if (savedUrl) {
          localStorage.setItem('catchat-beta-last-backup-url', savedUrl);
        }
  ...
  try {
    ...
    if (usePuter) {
      ...
      const backupUrl = resolvePuterReadUrl(
        pickedBackup,
        localStorage.getItem('catchat-beta-last-backup-url') || ''
      );
